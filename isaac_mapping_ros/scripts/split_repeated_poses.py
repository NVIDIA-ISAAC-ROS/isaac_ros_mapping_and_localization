...
"""
Clean script to split repeated pose files from cuvslam into individual run files.

This script:
1. Loads stereo.edex to determine original sequence length
...
class RepeatedPoseSplitter:
    ...
    def _load_frame_count(self) -> int:
        """Load original frame count from stereo.edex."""
        stereo_edx_path = self.edx_dir / 'stereo.edex'  # Correct extension
    ...
    def _create_timestamp_mapping(self, timestamps: np.ndarray) -> tuple:
        ...
        if len(timestamps) != expected_total:
            ...
            self.logger.error(f"  ({self.original_frame_count} frames × {self.repeat_count} runs)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Split repeated pose files from cuvslam into individual run files"
    )

    parser.add_argument('--poses_dir', type=pathlib.Path, required=True,
                        help='Directory containing repeated pose files')
    parser.add_argument('--edx_dir', type=pathlib.Path, required=True,
    ...
    if not args.poses_dir.exists():
        print(f"Error: Poses directory does not exist: {args.poses_dir}")
        sys.exit(EXIT_FILE_NOT_FOUND)

    if not args.edx_dir.exists():
        print(f"Error: EDX directory does not exist: {args.edx_dir}")
        sys.exit(EXIT_FILE_NOT_FOUND)
